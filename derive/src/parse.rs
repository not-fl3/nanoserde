...
pub fn parse_data(input: TokenStream) -> Data {
    ...
    fn next_visibility_modifier<T: Iterator<Item = TokenTree>>(
    ...
    fn next_punct<T: Iterator<Item = TokenTree>>(source: &mut Peekable<T>) -> Option<String> {
    ...
    fn next_exact_punct<T: Iterator<Item = TokenTree>>(
        source: &mut Peekable<T>,
        pattern: &str,
    ) -> Option<String> {
        if let Some(TokenTree::Punct(punct)) = source.peek() {
    ...
    fn next_eof<T: Iterator>(source: &mut Peekable<T>) -> Option<()> {
    ...
    fn next_ident<T: Iterator<Item = TokenTree>>(source: &mut Peekable<T>) -> Option<String> {
        if let Some(TokenTree::Ident(ident)) = source.peek() {
    ...
    fn next_group<T: Iterator<Item = TokenTree>>(source: &mut Peekable<T>) -> Option<Group> {
    ...
    fn next_attributes_list<T: Iterator<Item = TokenTree>>(
        source: &mut Peekable<T>,
    ) -> Vec<Attribute> {
